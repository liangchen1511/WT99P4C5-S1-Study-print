...
**当前版本：V1.2.1**（Ultimate 家长 Web 控制台 · 搜题记录下载照片）
...
- 👪 **家长站** — 浏览器 `/parent/` 控制台（策略、历史、聊天、相册同步）；设备端 policy、聊天未读角标（SDIO 负载已做错峰节流）
...
- 进入 AI助手时**保留状态栏**，仅暂停时钟刷新（避免藏栏整屏重排爆闪）；退出后立刻按系统时间安静重画（不改 TZ、不切 12/24 格式）。
- 退出 App 后搜题、家长聊等功能正常。
...
| 功能 | 说明 |
|------|------|
| **Web 控制台** | `http://<ECS公网IP>/parent/` — 总览、搜题历史、策略、聊天、相册/打印上传；一键部署 UI：`tools/soti-standalone-server/deploy-parent-ui.ps1`（详见 [PARENT_PORTAL.md](./tools/soti-standalone-server/PARENT_PORTAL.md)） |
| 相册 | SD 根目录 JPEG 列表、预览、删除 |
| 搜题 | 选中照片 → 跳转 SoTi 上传 |
| 同步 | 从家长站拉取待下载照片（后台 `alb_sync_bg`） |
| 策略 | `parent_policy` 控制 App 可用时段/开关 |
| 聊天 | `ParentChat` + 桌面未读角标；设备端 **拼音输入**（26 键 + 候选栏，字库见 `lv_font_ui_zh`） |
| 搜题历史 | SQLite 记录 + 可选缩略图；详情抽屉可 **下载照片**（V1.2.0） |

**亲子聊天时间：** 消息气泡显示「几月几日 时:分」（如 `7月3日 18:30`），设备端与家长站网页一致。

**亲子聊天输入：** 点击输入框弹出键盘。底部自下而上依次为 **键盘 → 输入框 → 候选栏**（拼音时显示）。输入框会先显示拼音字母（如 `ni`）→ 点 **候选栏** 选汉字 → 点「发送」。不会自动把字母变成中文，需手动选字。`i`/`u`/`v` 不能作为音节首字母，请用完整音节如 `yi`/`wu`/`nv`。词库由 `tools/gen_parent_chat_pinyin_dict.py` 按字库字集生成；改字集后需重跑脚本并 `idf.py build`。进入聊天自动滚到最新消息。
...
>- 播放前会释放 CSI 缓冲/DMA 与硬件 JPEG 编码器，并暂停家长后台 HTTP；若用过搜题/相机后仍无法播放，请重新编译烧录本修复后再开「视频」
