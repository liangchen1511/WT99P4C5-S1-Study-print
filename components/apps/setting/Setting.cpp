...
/** SoTi / HTTPS: replace DHCP DNS with public resolvers after STA gets IPv4. */
static void staGotIpDnsHandler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    (void)arg;
    (void)event_base;
    (void)event_id;
    ...
    if (event != nullptr && event->esp_netif != nullptr) {
        bsp_netif_apply_public_dns(event->esp_netif);
        /* SoTi / HTTPS: default netif was likely Ethernet (created first, no IP). Force outbound + DNS on STA. */
...
/** Auto-retry saved Wi-Fi after disconnect (0 = idle or connected). */
static TickType_t s_saved_wifi_next_reconnect = 0;
...
AppSettings::AppSettings():
    ESP_Brookesia_PhoneApp("设置", &img_app_setting, false),                  // auto_resize_visual_area
...
bool AppSettings::init(void)
{
    ...
    power_manager_set_backlight_percent((int)_nvs_param_map[NVS_KEY_DISPLAY_BRIGHTNESS]);
...
{
    /* Main */
    lv_label_set_text(ui_LabelPanelSettingMainContainer3Volume, "音量");
    lv_label_set_text(ui_LabelPanelSettingMainContainer4Light, "亮度");
...
esp_err_t AppSettings::initWifi()
{
    ...
    esp_event_handler_instance_t instance_sta_got_ip{};
    ret = esp_event_handler_instance_register(IP_EVENT,
                                            ...
                                            &instance_sta_got_ip);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Register IP_EVENT_STA_GOT_IP failed: %s", esp_err_to_name(ret));
        return ret;
    }
...
void AppSettings::scanWifiAndUpdateUi(void)
{
    ...
    if (ret != ESP_OK && ret != ESP_ERR_WIFI_CONN) {
        ESP_LOGW(TAG, "esp_wifi_start before scan failed: %s", esp_err_to_name(ret));
        return;
    }
    ...
    /* User may leave the scan screen or pick an AP while scan runs; do not call
     * esp_wifi_scan_stop() elsewhere against this blocking scan — ESP-Hosted RPC
     * can return NULL AP list and crash in hosted_memcpy (see rpc_wifi_scan_get_ap_records). */
    if (!(xEventGroupGetBits(s_wifi_event_group) & WIFI_EVENT_SCANING)) {
        return;
    }
...
void AppSettings::wifiScanTask(void *arg)
{
    ...
    if (app->_nvs_param_map[NVS_KEY_WIFI_ENABLE]) {
        ...
        if (esp_wifi_get_config(WIFI_IF_STA, &saved_cfg) == ESP_OK && saved_cfg.sta.ssid[0] != '\0') {
            strncpy(st_wifi_ssid, reinterpret_cast<const char *>(saved_cfg.sta.ssid), sizeof(st_wifi_ssid) - 1);
            st_wifi_ssid[sizeof(st_wifi_ssid) - 1] = '\0';
            strncpy(st_wifi_password, reinterpret_cast<const char *>(saved_cfg.sta.password), sizeof(st_wifi_password) - 1);
            st_wifi_password[sizeof(st_wifi_password) - 1] = '\0';
            ESP_LOGI(TAG, "Auto-connect to saved Wi-Fi: %s", st_wifi_ssid);
            esp_wifi_connect();
            s_saved_wifi_next_reconnect = xTaskGetTickCount() + pdMS_TO_TICKS(8000);
        }
    }
    ...
    while (true) {
        ...
        if (app->_nvs_param_map[NVS_KEY_WIFI_ENABLE] &&
            ...
            if (esp_wifi_get_config(WIFI_IF_STA, &saved_cfg) == ESP_OK && saved_cfg.sta.ssid[0] != '\0') {
                ...
                if (s_saved_wifi_next_reconnect != 0 && (int32_t)(now - s_saved_wifi_next_reconnect) >= 0) {
                    ESP_LOGI(TAG, "Retry connect to saved Wi-Fi: %s", (const char *)saved_cfg.sta.ssid);
                    esp_wifi_connect();
                    s_saved_wifi_next_reconnect = now + pdMS_TO_TICKS(12000);
                }
            }
        } else if (xEventGroupGetBits(s_wifi_event_group) & WIFI_EVENT_CONNECTED) {
            s_saved_wifi_next_reconnect = 0;
        }
...
void AppSettings::wifiConnectTask(void *arg)
{
    ...
    memcpy(st_wifi_password, lv_textarea_get_text(ui_TextAreaScreenSettingVerificationPassword), sizeof(wifi_config.sta.password));
...
void AppSettings::wifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_CONNECTED) {
        ...
        if (app->_nvs_param_map[NVS_KEY_WIFI_ENABLE]) {
            wifi_config_t dc = {};
            if (esp_wifi_get_config(WIFI_IF_STA, &dc) == ESP_OK && dc.sta.ssid[0] != '\0') {
                s_saved_wifi_next_reconnect = xTaskGetTickCount() + pdMS_TO_TICKS(3000);
            }
        }
    ...
    if (state & LV_STATE_CHECKED) {
        ...
        if (!(xEventGroupGetBits(s_wifi_event_group) & WIFI_EVENT_CONNECTED)) {
            ...
            if (esp_wifi_get_config(WIFI_IF_STA, &saved_cfg) == ESP_OK && saved_cfg.sta.ssid[0] != '\0') {
                ...
                strncpy(st_wifi_password, reinterpret_cast<const char *>(saved_cfg.sta.password), sizeof(st_wifi_password) - 1);
                st_wifi_password[sizeof(st_wifi_password) - 1] = '\0';
                ESP_LOGI(TAG, "Wi-Fi enabled, connecting to saved SSID: %s", st_wifi_ssid);
                esp_wifi_connect();
            }
        }
...
void AppSettings::onButtonWifiListClickedEventCallback(lv_event_t * e)
{
    ...
    xEventGroupClearBits(s_wifi_event_group, WIFI_EVENT_SCANING);
    /* Do not esp_wifi_scan_stop() here: wifiScanTask may be inside esp_wifi_scan_start /
     * get_ap_records; racing scan_stop corrupts ESP-Hosted RPC (hosted_memcpy assert). */
}
...
void AppSettings::onSliderPanelLightSwitchValueChangeEventCallback( lv_event_t * e) {
    ...
    if (brightness != app->_nvs_param_map[NVS_KEY_DISPLAY_BRIGHTNESS]) {
        ...
        power_manager_set_backlight_percent(brightness);
    }
