...
static bool s_sntp_stack_started;
...
static void obtain_time(void)
{
    ...
    /* Wait for calendar time — SNTP_SYNC_STATUS_* alone can race; wrong epoch breaks HTTPS certs. */
    time_t now = 0;
    ...
    for (int retry = 0; retry < retry_count; retry++) {
        ...
        localtime_r(&now, &timeinfo);
        if (timeinfo.tm_year >= (2016 - 1900)) {
            ESP_LOGI(TAG, "SNTP: wall clock valid (year >= 2016)");
            return;
        }
    }
    ESP_LOGW(TAG, "SNTP: time still invalid after %d attempts — TLS to HTTPS servers may fail (check UDP 123 / firewall)",
             retry_count);
}

static void initialize_sntp(void)
{
    if (s_sntp_stack_started) {
        return;
    }
