...
static void show_video_task(void *arg)
{
    ...
    /* HW JPEG decoder aligns output dimensions (see jpeg_decoder_process size check). */
    width = ALIGN_UP(width, 16);
    height = ALIGN_UP(height, 16);

    /* Decoder outputs RGB565 (jpeg_decode_cfg); LV_IMG_CF_TRUE_COLOR is 2 bytes/pixel here */
    bool need_new_out = (player_ctx.out_buff == NULL || player_ctx.video_width != width ||
                         player_ctx.video_height != height);
    if (need_new_out) {
        uint32_t out_sz = width * height * (uint32_t)sizeof(lv_color_t);
        uint8_t *new_out = video_decoder_malloc(out_sz, false, &out_sz);
        ESP_GOTO_ON_FALSE(new_out, ESP_ERR_NO_MEM, err, TAG, "Allocation out_buff failed");
        if (player_ctx.out_buff != NULL) {
            heap_caps_free(player_ctx.out_buff);
            player_ctx.out_buff = NULL;
            player_ctx.out_buff_size = 0;
        }
        player_ctx.out_buff = new_out;
        player_ctx.out_buff_size = out_sz;
...
err:
    ...
    /* Keep out_buff: lv_canvas still references it until next play or esp_lvgl_simple_player_del(). */
...
void esp_lvgl_simple_player_play(void)
{
    ...
    if (player_ctx.state == PLAYER_STATE_STOPPED) {
        /* After pause_background(), task may still be tearing down — release LVGL so it can finish. */
        if (player_task_handle != 0) {
            bsp_display_unlock();
            if (esp_lvgl_simple_player_wait_task_stop(-1) != ESP_OK) {
                ESP_LOGE(TAG, "Wait task stop before play failed");
            }
            bsp_display_lock(0);
        }
...
void esp_lvgl_simple_player_pause_background(void)
{
    if (!player_ctx.is_init) {
        return;
    }
    ...
    if (player_task_handle != 0) {
        bsp_display_unlock();
        if (esp_lvgl_simple_player_wait_task_stop(-1) != ESP_OK) {
            ESP_LOGE(TAG, "pause_background wait stop failed");
        }
        bsp_display_lock(0);
    }
}
...
esp_err_t esp_lvgl_simple_player_del(void)
{
    ...
    if (player_ctx.out_buff != NULL || player_ctx.display_buff != NULL) {
        ...
        if (player_ctx.canvas != NULL && lv_obj_is_valid(player_ctx.canvas)) {
