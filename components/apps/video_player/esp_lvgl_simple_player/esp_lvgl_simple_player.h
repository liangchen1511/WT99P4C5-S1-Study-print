...
/**
 * @brief Pause player (in-app: decoding task keeps running, waits in paused loop)
 */
void esp_lvgl_simple_player_pause(void);

/**
 * @brief Full stop for multitasking: stops decode task and waits for exit.
 * Call from LVGL context while lvgl_port_lock is held (e.g. Brookesia processPause).
 */
