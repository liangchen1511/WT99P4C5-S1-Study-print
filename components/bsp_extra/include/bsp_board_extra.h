...
/**
 * @brief Play a short camera shutter sound asynchronously (non-blocking).
 *
 * Uses the speaker codec path initialized by bsp_extra_codec_init().
 * Safe to call from any task; playback runs on a dedicated FreeRTOS task.
 *
 * @return ESP_OK if playback was queued, ESP_ERR_INVALID_STATE if codec not init
 */
