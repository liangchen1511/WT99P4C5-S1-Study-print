...
void bsp_netif_apply_public_dns(esp_netif_t *netif)
{
    if (netif == NULL) {
        return;
    }
    ...
    if (esp_netif_str_to_ip4("223.5.5.5", &dns.ip.u_addr.ip4) == ESP_OK) {
        esp_netif_set_dns_info(netif, ESP_NETIF_DNS_MAIN, &dns);
    }
    if (esp_netif_str_to_ip4("119.29.29.29", &dns.ip.u_addr.ip4) == ESP_OK) {
        esp_netif_set_dns_info(netif, ESP_NETIF_DNS_BACKUP, &dns);
    }
    ESP_LOGI(TAG, "Applied public DNS (223.5.5.5 / 119.29.29.29) on interface");
}
